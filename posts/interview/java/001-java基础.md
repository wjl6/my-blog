...
### 3. volatile 的作用？
- **核心作用**：volatile **保证变量的内存可见性、禁止指令重排序**，但**不保证原子性**；
...
> 线程池队列的核心价值是「**缓冲任务、减少线程创建、平衡系统负载**」，不同类型的队列对应不同的业务场景，生产环境的关键是「选择有界队列，避免无界队列引发 OOM」

| 队列类型               | 实现方式 | 有界 / 无界       | 排序规则       | 并发性能       | 内存风险               | 生产推荐度               |
|------------------------|----------|-------------------|----------------|----------------|------------------------|--------------------------|
| ArrayBlockingQueue     | 数组     | 强制有界          | FIFO           | 稳定（中）     | 无（可控）             | ✅ ✅ ✅ （首选）|
| LinkedBlockingQueue    | 链表     | 可选（默认无界）| FIFO           | 高（分离锁）| 有（无界模式 OOM）| ⚠️ （仅指定容量后可用）|
| SynchronousQueue       | 无存储   | 无容量            | 无（直接传递） | 极致高         | 无（无缓冲）| ✅ （仅特殊场景）|
| PriorityBlockingQueue  | 堆       | 无界              | 优先级         | 中（排序开销） | 有（OOM）| ⚠️ （仅优先级场景）|
